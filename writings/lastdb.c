...
static int decay_value_at(const char *s, size_t n, double now, double *out)
{
    ...
    *t2 = 0;
    ...
    double floor = __builtin_fabs(val) * 0x1.0p-10;
    if (age > hl * 32 || __builtin_fabs(x) <= floor) {
        *out = 0;
        return 1;
    }
    *out = x;
...
static inline int srv_read_lock_func(const char *db_path) {
    ...
    if (needs_reopen || ht_len > ht_sorted_len + 65536 || (ht_sorted_len && ht_len > ht_sorted_len * 17 / 16)) {
        ...
        struct stat st2, fd2;
        needs_reopen = (srv_db_fd < 0) || (!stat(db_path, &st2) && (!fstat(srv_db_fd, &fd2) ? 1 : (st2.st_ino != fd2.st_ino || st2.st_size > (off_t)map_size)));
        if (needs_reopen) {
            if (srv_db_fd >= 0 && !fstat(srv_db_fd, &fd2) && st2.st_ino != fd2.st_ino) { close(srv_db_fd); srv_db_fd = -1; }
            load_db(db_path);
...
static inline void srv_unlock_read_attr(int *dummy) { (void)dummy; pthread_rwlock_unlock(&srv_rw); }
static inline void srv_unlock_write_attr(int *dummy) { (void)dummy; pthread_rwlock_unlock(&srv_rw); }
...
double w_ratio = raw_count > 0 ? count_est / (double)raw_count : 1.0;
if (w_ratio < 1.0) ess *= w_ratio;
double ess_conf = ess >= 1000 ? 1.0 : ess / 1000.0;
double decay_conf = raw_count > 0 ? ac_sum / (double)raw_count : 1.0;
if (w_ratio > 1.0) decay_conf /= w_ratio;
double confidence = ess_conf * decay_conf;
                                ...
                                if (ess < 30.0 * (double)(1 + max_wl) || confidence < 0.02) { count_est = 0; sum = 0; }
...
double conf = w_ratio > 1.0 ? 1.0 / w_ratio : w_ratio;
                if (raw_s < 5) conf *= (double)raw_s / 5.0;
                if (conf < 0.05 * __builtin_exp2((double)max_wl / 4.0)) s = 0;
